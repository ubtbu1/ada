...
\usepackage{placeins}  % 添加 placeins 包用于 \FloatBarrier
\usepackage{float}     % 添加 float 包用于更好的浮动体控制
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \begin{lstlisting}[style=algorithmPPT]
            BFS(3)
                ...
                3.color:=GRAY; // 3 染灰
                3.dis:=0; // 3 dis=0
                queNode.EUQUE(3); // 3 入队
                while queNode != empty do
                    w:=queNode.DEQUE(); // 3 出队
                    foreach neighbor [5,6] of 3 do
    ...
    \item \textbf{第二层（dis = 1）：}
        ...
        \begin{lstlisting}[style=algorithmPPT]
                while queNode = [5,6] != empty do // 数字序 5<6
                    w:=queNode.DEQUE(); // 5 出队
                    foreach neighbor [4] of 5 do
        ...
        \begin{lstlisting}[style=algorithmPPT]
                while queNode = [6,4] != empty do
                    w:=queNode.DEQUE(6); // 6 出队
                    foreach neighbor [6] of 6 do // 6 邻居是自己
    ...
    \begin{lstlisting}[style=algorithmPPT]
                while queNode = [4] != empty do
                    w:=queNode.DEQUE(); // 4 出队
                    foreach neighbor [2] of 4 do 
    ...
    \begin{lstlisting}[style=algorithmPPT]
        while queNode = [2] != empty do
            w:=queNode.DEQUE(); // 2 出队
            foreach neighbor [5] of 2 do 
    ...
    \begin{itemize}
        ...
        \begin{lstlisting}[style=algorithmPPT]
            BFS(u)
                ...
                while queNode != empty do
                    u:=queNode.DEQUEUE(); // u 出队
                    foreach neighbor [t,y] of u do
                        if t.color==WHITE then
                            t.color:=GRAY; // t 染灰
                            t.parent:=u; // t parent=u
                            t.dis:=3.dis+1; // t dis=1
    ...
    \item \textbf{第二层（dis = 1）：}
        ...
        \begin{lstlisting}[style=algorithmPPT]
                while queNode = [t,y] != empty do 
                    t:=queNode.DEQUE(); // t 出队
                    foreach neighbor [u,w,x] of t do
                        if u.color==WHITE then//False
        ...
        \begin{lstlisting}[style=algorithmPPT]
                while queNode = [y,w,x] != empty do
                    w:=queNode.DEQUE(y); // y 出队
                    foreach neighbor [u,x] of y do 
                        if u.color==WHITE then // False
                        if x.color==WHITE then // False
...
\end{tabular}
    ...
    \begin{lstlisting}[style=algorithmPPT]
                while queNode = [w,x] != empty do
                    w:=queNode.DEQUE(); // w 出队
                    foreach neighbor [s,t,x] of w do
    ...
    \begin{lstlisting}[style=algorithmPPT]
                while queNode = [x,s] != empty do
                    x:=queNode.DEQUE(); // x 出队
                    foreach neighbor [t,w,y] of x do 
                        if t.color==WHITE then // False
                        if w.color==WHITE then // False
                        if y.color==WHITE then // False
    ...
    \begin{lstlisting}[style=algorithmPPT]
                while queNode = [s] != empty do 
                    s:=queNode.DEQUE(); // s 出队
                    foreach neighbor [r,w] of s do
    ...
    \begin{lstlisting}[style=algorithmPPT]
                while queNode = [r] != empty do 
                    r:=queNode.DEQUE(); // r 出队
                    foreach neighbor [s,v] of r do
                        if s.color==WHITE then //False
                        if v.color==WHITE then
                            v.color:=GRAY; // v 染灰
    ...
    \begin{lstlisting}[style=algorithmPPT]
        while queNode = [v] != empty do 
            v:=queNode.DEQUE(); // v 出队
            foreach neighbor [r] of v do
                if r.color==WHITE then //False
